...
println "Ensuring starting state: dropping any existing non-_id indexes so we can measure non-indexed performance..."
try {
    historical.dropIndexes()
    general.dropIndexes()
    println "Existing indexes dropped (except _id)."
} catch (e) {
    println "Warning: could not drop indexes: ${e.message}"
}
println ""
...
// Run without indexes
println "\nRunning aggregation WITHOUT indexes..."
long startNoIdx = System.nanoTime()
def resultsNoIdx = historical.aggregate(pipeline).into([])
long endNoIdx = System.nanoTime()
double timeNoIdx = (endNoIdx - startNoIdx) / 1_000_000_000.0
println "Aggregation WITHOUT indexes: ${String.format('%.3f', timeNoIdx)} seconds\n"

// Create indexes and run again
...
println "Running aggregation WITH indexes..."
long startIdx = System.nanoTime()
def results = historical.aggregate(pipeline).into([])
long endIdx = System.nanoTime()
double timeIdx = (endIdx - startIdx) / 1_000_000_000.0
println "Aggregation WITH indexes: ${String.format('%.3f', timeIdx)} seconds\n"

println "Summary: WITHOUT indexes = ${String.format('%.3f', timeNoIdx)}s, WITH indexes = ${String.format('%.3f', timeIdx)}s\n"

// Print results once (from indexed run)
results.each { doc ->
    def formattedDoc = [
